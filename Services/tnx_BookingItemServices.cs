        ...
        async Task<ServiceStatusResponseModel> Itnx_BookingItemServices.GetResultEntryAllData(ResultEntryAllDataRequestModel resultentrydata)
        {
            var query = from tb in db.tnx_Booking
                        ...
                        select new
                        {
                            ...
                            (tbi.isApproved == 0 && tbi.isResultDone == 0 && tbi.isSampleCollected == "Y") ? "Sample Collected" :
                            ...
                            (db.tnx_OutsourceDetail.Where(o=>o.testId==tbi.id).Count()>0)? "Out Source" :
                             tbi.isUrgent == 1 ? "Urgent Sample" : "Pending Report",
                        };
